...
function ItemsTab({ isAdmin = false, items, isLoading, onUploadSuccess }: ItemsTabProps) {
  ...
  const handleDownload = async (fileName: string) => {
    if (!fileName) return
    try {
      const res = `${import.meta.env.PUBLIC_API_URL_LOCAL}/api/download-url?filename=${fileName}&action=view`
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 1. 署名付きURL取得
      const urlRes = await fetch(`${import.meta.env.PUBLIC_API_URL_LOCAL}/api/upload-url?filename=${encodeURIComponent(file.name)}&contentType=${encodeURIComponent(contentType)}`);
      ...
      // 3. GoのDB保存APIを叩く
      await fetch(`${import.meta.env.PUBLIC_API_URL_LOCAL}/api/save-material`, {
        method: 'POST',
