...
import { Sun, Moon, Monitor, Settings, X, LogIn } from 'lucide-react';
                ...
                <div className="p-4 sm:p-8 h-full">
                  {activeTab === 'Tab1' && (
                    <Suspense fallback={<WelcomeSkeleton />}>
                      <WelcomeTab isLoading={isLoading} news={news} locale={locale} />
                    </Suspense>
                  )}
                  {activeTab === 'Tab2' && (
                  ...
                  {activeTab === 'Tab4' && (
                    <Suspense fallback={<QASkeleton />}>
                      <QATab isAdmin={isAdmin} isLoading={isLoading} questions={questions} onQuestionSubmit={refreshQuestions} locale={locale} />
                    </Suspense>
                  )}
                </div>
              </main>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {(['light', 'dark', 'system'] as const).map((mode) => (
                        ...
                        >
                          {mode === 'light' && <Sun className={`w-5 h-5 ${theme === mode ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`} />}
                          {mode === 'dark' && <Moon className={`w-5 h-5 ${theme === mode ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`} />}
                          {mode === 'system' && <Monitor className={`w-5 h-5 ${theme === mode ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`} />}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {([['ja', '日本語'], ['en', 'English']] as const).map(([code, label]) => (
                        ...
                        >
                          <span className="text-lg">{code === 'ja' ? '🇯🇵' : '🇺🇸'}</span>
